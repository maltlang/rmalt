...
impl ThreadContext {
	...
	pub fn from(boot_module: ModuleContext) -> ThreadContext {
		let module = Handle::from(system_module());
		{
			let mut hs = boot_module.vtab.borrow_mut();
			hs.insert("--name--".to_string(), Value::String(Handle::from("--main--".to_string())));
			hs.insert("Prelude".to_string(), Value::Module(Handle::from(module.clone())));
		}
		...
		ThreadContext {
			system_mod: module,
			using_mod: RefCell::from(Arc::from(boot_module)),
			framestack: RefCell::from(vec![RefCell::from(None); 256]),
			frame_size: RefCell::from(0),
		}
	}

	pub fn from_args(_argv: &[String]) -> ThreadContext {
		unimplemented!()
	}

	pub fn load_module_from_path(&self, _path: String) {}

	pub fn load_module(&self, _module: ModuleContext) {}
